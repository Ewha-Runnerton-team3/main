...
router.get('/kakao/callback', async (req, res) => {
    try {
      ...
      const clientRedirectUrl = `http://localhost:5173/auth/kakao/callback?token=${encodeURIComponent(token)}&loginId=${encodeURIComponent(user.loginId)}&nickname=${encodeURIComponent(user.nickname)}`;
